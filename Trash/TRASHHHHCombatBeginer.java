...
public class TRASHHHHCombatBeginer extends Script {
	...
	public void Checkup() throws InterruptedException {
		...
		// if (inventory.contains("Bronze sword") || (inventory.contains("Wooden
		// shield")) && ){
		...
		if (inventory.contains("Bronze sword")
				|| (inventory.contains("Wooden shield")) && (!inventory.contains("Pot"))) {
			log("Not Fresh");
			Equip();
		} else if (!myPlayer().isUnderAttack() && Cows.contains(myPlayer()) && !myPlayer().isAnimating()) {
			log("Loot");
			Loot();
		} else if (inventory.contains("Bronze sword") && (inventory.contains("Wooden shield"))
		...
		} else if (Cows.contains(myPlayer()) && !getCombat().isFighting()
				&& equipment.isWearingItem(EquipmentSlot.WEAPON, "Bronze Sword")
				&& equipment.isWearingItem(EquipmentSlot.SHIELD, "Wooden shield")) {
			log("You are in area and redy to kill");
			sleep(random(random(3000, 4123)));
			Killcows();
		} else if (!Cows.contains(myPlayer()) && equipment.isWearingItem(EquipmentSlot.WEAPON, "Bronze Sword")
				&& !inventory.contains("Pot")) {
			Gotoarea();

		} else if (equipment.isWearingItem(EquipmentSlot.WEAPON, "Bronze shield")
				&& equipment.isWearingItem(EquipmentSlot.SHIELD, "Wooden shield")) {
			log("You got all lets go");
			Gotoarea();
		} else if (!getInventory().contains("Bronze sword") && Banks.LUMBRIDGE_UPPER.contains(myPlayer())
	...
	private void Killcows() throws InterruptedException {
		...
		if (enemy != null && enemy.exists() && !(enemy.getHealthPercent() > 0) && !getCombat().isFighting()
				&& !enemy.isUnderAttack() && !myPlayer().isUnderAttack() && !myPlayer().isAnimating()) {
			log("Attacking");
			enemy.interact("Attack");
			sleep(random(1500, 2240));
			while (enemy != null && enemy.exists() && enemy.getHealthPercent() < 0 && enemy.isUnderAttack()
					&& myPlayer().isUnderAttack() && myPlayer().isAnimating()) {
				sleep(random(1500, 2240));
