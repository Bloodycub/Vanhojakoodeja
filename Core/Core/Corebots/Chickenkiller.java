...
public class Chickenkiller extends Skeleton {
	...
	public void onStart() throws InterruptedException {
		...
		if (myPlayer().getCombatLevel() <= randomlevel) {
			...
			ItemChecklist();
			if (inventory.getEmptySlots() <= 13 && !Areas.Chickenpit.contains(myPlayer())) {
	...
	private void CheckUp() throws InterruptedException {
		...
		if (settings.getRunEnergy() >= Runspeedrandomaiser && !getSettings().isRunning()) {
			...
			log("Cheking items");
			ItemChecklist();
		} else if (!areas.Chickenpit.contains(myPlayer())) {
			log("Walking to Spot");
			Walkingpit();
		} else if (inventory.isFull() && inventory.contains("Bones")) {
			log("Bury");
			Buring();
		} else if (Areas.Chickenpit.contains(myPlayer()) && !myPlayer().isMoving() && !myPlayer().isUnderAttack()
	...
	private void Kill() throws InterruptedException {
		...
		if (Chicken != null && !Chicken.isHitBarVisible() && Areas.Chickenpit.contains(Chicken)) {
	...
	private void Walkingpit() throws InterruptedException {
		log("Walking to pit");
		if (!Areas.Chickenpit.contains(myPlayer())) {
			walking.webWalk(areas.Chickenpit());
