...
public class Woodcutter extends Script {
    ...
    final String[] typesOfLogs = {"Logs","Oak logs","Willow logs","Maple logs","Mahogany logs","Yew logs","Magic logs"};
...
final int[] XP_TABLE = { 
    ...
    public void Checkup() throws InterruptedException {
    	if (!inventory.isEmpty() && !inventory.contains("Bronze axe") && !Banks.LUMBRIDGE_UPPER.contains(myPlayer()) && (!inventory.contains("Bronze axe")) && !woodcuttingspot.contains(myPlayer())) {
    		log("Walking to bank with inventory not empty");
            walking.webWalk(banks);
            Atlbbank();
    	}
    	else if (woodcuttingspot.contains(myPlayer()) && inventory.contains("Oak logs")) {
    		log("Droping oaks");
        	...
        	log("No axe but in bank area and got pot");
            Atlbbank();
        } else if (Banks.LUMBRIDGE_UPPER.contains(myPlayer()) && !inventory.contains("Bronze axe")) {
        	log("At bank and inventory empty");
            Atlbbank();
        } else if (inventory.contains("Logs") && inventory.isFull() && woodcuttingspot.contains(myPlayer())) {
            log("Droping wood");
            Dropwood();
        } else if (inventory.isFull() && inventory.contains("logs")) {
            log("Stuck");
            log("You dont have wood and inventory not full cut wood");
            Cutwood();
    ...
    //	if (!getInventory().contains("Pot") && (getInventory().contains("Bronze axe"))){
