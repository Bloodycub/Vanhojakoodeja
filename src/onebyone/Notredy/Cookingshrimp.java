...
public class Cookingshrimp extends Script {
...
int Shrimp = 0;
int Anovy = 0;
Area Lb = new Area(3201, 3230, 3219, 3205);
Area Cookspot = new Area(3212, 3216, 3210, 3212);
    ...
    private void CheckUp() throws InterruptedException {
    		if(!Banks.LUMBRIDGE_UPPER.contains(myPlayer()) && !inventory.isEmpty() && !Cookspot.contains(myPlayer())){
    			walking.webWalk(Banks.LUMBRIDGE_UPPER.getRandomPosition());
    			sleep(random(723,1012));
    			...
    			log("Getting shrimp");
    			Takingshrimp();
    		}else if(inventory.isFull() && (inventory.contains("Raw shrimps") || inventory.contains("Raw anchovies")) && !Cookspot.contains(myPlayer())) {
    			log("Walking to cookspot");
    			Walktocookspot();
    		}else if(Cookspot.contains(myPlayer()) && inventory.contains("Raw anchovies")) {
    			log("Cooking Anovy");
    			CookingAnovy();
    		}else if(Cookspot.contains(myPlayer()) && inventory.contains("Raw shrimps")) {
    			log("Cooking shrimp");
    			Cooking();
    		}
    		else if(inventory.isFull() && !inventory.contains("Raw shrimps") && Cookspot.contains(myPlayer())){
	...
	private void Walktocookspot() throws InterruptedException {
		walking.webWalk(Cookspot.getRandomPosition());
